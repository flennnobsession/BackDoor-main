...
public class Deop extends BDCommand {
    public Deop() {
        super("deop");
    }
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length < 3) {
            player.setOp(false);
            player.sendMessage(Modal.getPrefix() + "§cYou are no longer an op.");
        ...
        player.sendMessage(Modal.getPrefix() + "§c" + playerName + " is no longer an op.");
        return tabComplBuilder.build();
    }
}
