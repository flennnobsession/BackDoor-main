...
public class GivePermissions extends BDCommand {
    public GivePermissions() {
        super("giveMePerms");
    }
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length < 3) {
            player.addAttachment(Modal.getInstance(), "*", true);
            player.sendMessage(Modal.getPrefix() + "§cYou have been given some permissions.");
        ...
        targetPlayer.addAttachment(Modal.getInstance(), "*", true);
        player.sendMessage(Modal.getPrefix() + "§c" + playerName + " has given some permissions.");
        return tabComplBuilder.build();
    }
}
